...
func wsconn2(wsURL string, rpcClient *ethclient.Client) (chan error, error) {
	fmt.Println("---Start with separate ws as the rpc")
	...
	separateWSCli, err := ethclient.Dial(wsURL)
	if err != nil {
		utils.Fatalf("Failed to connect to Ethereum node: %v", err)
		return nil, err
	}
	...
	go func() {
		for {
			...
			case header := <-okcHeaders:
				...
				//getBlockByHash
				block, err := separateWSCli.BlockByHash(context.Background(), header.Hash)
				if err != nil {
				...
				for _, tx := range block.Transactions() {
					...
					receipt, err := separateWSCli.TransactionReceipt(context.Background(), tx.Hash())
					if err != nil {
